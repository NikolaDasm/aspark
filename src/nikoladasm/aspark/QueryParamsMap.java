...
public class QueryParamsMap {
	...
	public QueryParamsMap get(String key) {
		return queryMap.getOrDefault(key, new QueryParamsMap());
	}
	
	public QueryParamsMap createIfAbsentAndGet(String key) {
		queryMap.putIfAbsent(key, new QueryParamsMap());
		return queryMap.get(key);
	}
	
	public QueryParamsMap get(String... keys) {
		QueryParamsMap root = this;
		for (String key : keys) {
			root = get(key);
		}
		return root;
